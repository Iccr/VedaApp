...
DEFAULT
    ...
    attributes.each do |attribute|
      ...
      if attribute.is_array
        ...
        attribute_literal = "\t\t#{get_attribute_literal_prefix}var #{attribute.name.camelize}: #{attribute.type_name}#{default_value}\n"
...
@parsed.each do |class_name, attributes|
  ...
  attributes.each do |attribute|
    ...
    key_literal = ""
    if attribute.is_array
      attribute_literal = "\tlet #{attribute.name.camelize}: [#{attribute.type_name}]\n"
    else
      attribute_literal = "\tlet #{attribute.name.camelize}: #{attribute.type_name}\n"
    end
...
\t@Expose
\tprivate List<#{attribute.type_name}> #{attribute.name.camelize} = null;\n
Attribute
      getter_setter_literal = <<-GETTER
\tpublic List<#{attribute.type_name}> get#{attribute.name.camelize.cap_capitalize}() {
\t\treturn #{attribute.name.camelize};
\t}

\tpublic void set#{attribute.name.camelize.cap_capitalize}(List<#{attribute.type_name}> #{attribute.name.camelize}) {
...
\t@SerializedName(\"#{attribute.name}\")
\t@Expose
\tprivate #{attribute.type_name} #{attribute.name.camelize};\n
Attribute

    getter_setter_literal = <<-GETTER
\tpublic #{attribute.type_name} get#{attribute.name.camelize.cap_capitalize}() {
\t\treturn #{attribute.name.camelize};
\t}

\tpublic void set#{attribute.name.camelize.cap_capitalize}(#{attribute.type_name} #{attribute.name.camelize}) {
\t\tthis.#{attribute.name.camelize} = #{attribute.name.camelize};
\t}\n
GETTER
